    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.is_secret == true) {
          ...
          aHtml.push('<p class="qna-secret-pwd"><input type="password" id="secure_password" name="secure_password" onkeydown="if (event.keyCode == 13) ' + data.action_pass_submit + '"> <input type="button" value="확인" onclick="' + data.action_pass_submit + '" class="btnNormal"></p>');
          aHtml.push('</div>');
          aHtml.push('</form>');
        } else {
          // 글 내용
          ...
          if (data.comment != undefined) {
            aHtml.push(
              '<div class="gLeft">' +
              '<div class="qna-comment-read">' +
              '<a href="#none" class="btnNormal sizeS" onclick="QNA.comment_view(' + data.read['no'] + ');">' +
              '댓글보기 <em>' + data.read['comment_count'] + '</em>' +
              '<i aria-hidden="true" class="icon icoArrowBottom"></i>' +
              '</a>' +
              '</div>' +
              '<div class="qna-action-buttons">' +
              '<a href="#none" class="btnNormal sizeS" onclick="QNA.comment_write(this);">쓰기</a>'
            );

            if (data.write_auth == true) {
              aHtml.push(
                '<a href="#none" onclick="EC_PRODUCT_FRONT_BOARD_QNA.modify(' + data.no + ', ' + iProductNo + ');" class="btnNormal sizeS">수정</a>'
              );
            }
