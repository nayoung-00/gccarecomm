...
function fixedHeader() {
    var header = document.getElementById('header');
    var fixedMargin = document.getElementById('contents');

    // 필요한 요소가 없으면 실행 중단
    if (!header || !fixedMargin) return;

    var scrollY =
        window.pageYOffset ||
        document.documentElement.scrollTop;

    var headerHeight = header.scrollHeight + 'px';

    if (scrollY > header.offsetTop) {
        header.classList.add('fixed');
        fixedMargin.style.marginTop = headerHeight;
    } else {
        header.classList.remove('fixed');
        fixedMargin.style.marginTop = '0px';
    }
}
...
function bottomNav() {
    ...
    window.addEventListener('scroll', function () {
        ...
        // 페이지 최하단에 도달하면 하단 내비게이션 표시
        var documentHeight = document.documentElement.scrollHeight;
        var viewportHeight = window.innerHeight;

        if (scroll + viewportHeight >= documentHeight - 1) {
            nav.classList.remove('bottom-nav--hide');
        }
...
jQuery(document).ready(function() {
    ...
    if (header && projectTitle) {
        var headerHeight = header.scrollHeight;

        jQuery(projectTitle).css(
            'top',
            -headerHeight + 30
        );
    }
